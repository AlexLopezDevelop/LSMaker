...
class JoystickViewController: UIViewController {
    ...
    var joystickUtils: JoystickUtils = JoystickUtils()
    var lsMakerJoystick: LSMakerJoystick = LSMakerJoystick()

    override func viewDidLoad() {
        super.viewDidLoad()

        joystickMove.trackingHandler = { [self] joystickData in
            lsMakerJoystick.angle = Int(joystickData.angle)
            lsMakerJoystick.x = Int(joystickData.velocity.x)
            lsMakerJoystick.y = Int(joystickData.velocity.y)
            let turnDirection = joystickUtils.getDirectionFromParams(lsMakerJoystick: lsMakerJoystick)
            print("Trun: " + turnDirection)
            drivingDataManager.setDirection(direction: turnDirection)
        }
    }
